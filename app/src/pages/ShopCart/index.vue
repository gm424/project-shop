      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <button @click="deleteCartById(cart)">删除</button>
            <button>移到收藏</button>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      //派发action通知服务器修改个数
      //目前disNum形参：+（1） -（-1）input最终的个数（并不是变化量）
      //cart：哪一个产品身上有id
      //向服务器发请求修改数量
      switch (type) {
        //加号
   ...
   async updateChecked(cart,event){
     try{
      //带给服务器的参数isChecked不是布尔值应该是0|1
      //event.target.checked返回的是布尔值
